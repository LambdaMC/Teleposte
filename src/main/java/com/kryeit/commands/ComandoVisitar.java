...
public class ComandoVisitar implements CommandExecutor {
    ...
    private static boolean handleVisitPlayer(Player player, String arg, int height, int postX, int postZ) {
        ...
        if (targetPlayer == null && player.hasPermission("telepost.visit.others")) {
            ...
            Optional<Home> offlineHome = Teleposte.getDB().getHome(offlinePlayer.getUniqueId());
            ...
            boolean isInvited = Teleposte.getInstance().invites.getValues(playerID).contains(player.getUniqueId());

            if (player.hasPermission("telepost.visit.others")) {
                Optional<Home> home = Teleposte.getDB().getHome(playerID);
                if (home.isPresent()) {
                ...
                Optional<Home> invitedHome = Teleposte.getDB().getHome(playerID);
