...
public class ComandoNombrarPoste implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        Optional<Post> post = Teleposte.getDB().getPost(PostAPI.getPostID(args));
        if (post.isPresent()) {
            player.sendMessage(PostAPI.getMessage("named-post-already-exists").replace("%POST_NAME%", post.get().name()));
            return false;
        }

        for (Post p : Teleposte.getDB().getPosts()) {
        ...
        if (!player.hasPermission("telepost.namepost")) {
            ...
            if(Teleposte.getInstance().playerNamedPosts.getHashMap().containsValue(player.getUniqueId())) {
        ...
        Teleposte.getDB().addPost(new Post(PostAPI.nameToId(postName), postName, new Location(player.getWorld(), postX, 0, postZ, 0, 0)));
        player.sendMessage(PostAPI.getMessage("name-post").replace("%POST_NAME%", postName));
        if(!player.hasPermission("telepost.namepost")) {
            try {
                Teleposte.getInstance().playerNamedPosts.addElement(postName,player.getUniqueId());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
