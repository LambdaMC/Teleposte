    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!(sender instanceof Player player)) {
            ...
            if (args.length == 0) {
                ...
                for (Post post : Teleposte.getDB().getPosts()) {
                    Location loc = post.location();
                    if (loc.getBlockX() == postX && loc.getBlockZ() == postZ) {
                        Teleposte.getDB().deletePost(post.id());
                        ...
                        try {
                            Teleposte.getInstance().playerNamedPosts.deleteElement(post.name());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
            ...
            if (args.length >= 1) {
                ...
                if (post.isPresent()) {
                    Teleposte.getDB().deletePost(postID);
                    ...
                    try {
                        Teleposte.getInstance().playerNamedPosts.deleteElement(postName);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
