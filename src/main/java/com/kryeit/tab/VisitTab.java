...
public class VisitTab implements TabCompleter {
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        ...
        if (player.hasPermission("telepost.visit.others") && size == 1) {
            if(Teleposte.getInstance().offline.isEmpty()) {
                for (OfflinePlayer p : Bukkit.getOfflinePlayers()) {
                    if (Teleposte.getDB().getHome(p.getUniqueId()).isPresent()) Teleposte.getInstance().offline.add(p.getName());
                }
            }
            else {
                allTabs.addAll(Teleposte.getInstance().offline);
            }
        }
